...
export default function HeaderTransparentGhost() {
  ...
  const { open: openBooking } = useBookingTrigger();
        ...
        <div className="flex items-center justify-between px-6 md:px-10 py-5 md:py-6">
          ...
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={() => openBooking()}
              className="hidden md:inline-flex font-display text-xs uppercase tracking-[0.2em] text-white border-b border-white/30 hover:border-primary hover:text-primary transition-colors pb-0.5"
            >
              Book a visit →
            </button>
            <button
...
function MobileOverlay({ onClose, onBook }: { onClose: () => void; onBook: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className="p-6">
        <button
          type="button"
          onClick={onBook}
          className="w-full rounded-full bg-primary px-6 py-4 font-display text-sm font-semibold uppercase tracking-[0.15em] text-ink"
        >
          Book a visit
        </button>
      </div>
